          ...
          <table class="items-table">
            ...
            <tbody>
              <tr v-for="item in filteredItems" :key="item.ReportId">
                ...
                <td>
                  <span :class="['status-tag', item.ItemType.toLowerCase()]">
                    {{ item.ItemType }}
                  </span>
                </td>
              </tr>
              <tr v-if="filteredItems.length === 0">
                <td colspan="5">No found items match the selected filters.</td>
              </tr>
            </tbody>
          </table>
        </div>
  ...
  mounted() {
    ...
    fetch('http://localhost:3000/api/found-items')
      ...
      .then(data => {
        ...
        this.filteredItems = data; // Initially, set filteredItems to all fetched data
      })
